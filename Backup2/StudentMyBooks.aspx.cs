...
namespace prjLibrarySystem
{
    public partial class StudentMyBooks : System.Web.UI.Page
    {
        ...
        private void LoadMyBooks()
        {
            ...
            try
            {
                // Show all transactions for this member that are not cancelled:
                // - Pending approval  (RequestStatus=Pending)
                // - Active / Overdue  (RequestStatus=Accepted, Status=Active/Overdue)
                // - Pending return    (RequestType=Return, RequestStatus=Pending)
                // - Returned          (Status=Returned)
                // Each row aliased to match the grid's DataField names
                string query = @"
                    SELECT
                        t.BorrowID                                          AS LoanId,
                        b.Title,
                        b.Author,
                        t.BorrowDate,
                        t.DueDate,
                        ...
                        t.RequestStatus,
                        t.Status,
                        -- DisplayStatus drives badge colour and label
                        CASE
        ...
        private void ShowDetailModal(int borrowId)
        {
            ...
            {
                string memberIdStr = Session["MemberID"]?.ToString() ?? "";
                int memberId = string.IsNullOrEmpty(memberIdStr) ? 0 : Convert.ToInt32(memberIdStr);
        ...
        private void HandleReturn(int borrowId)
        {
            try
            {
                // Verify this transaction belongs to the logged-in student
                // and is currently Active (accepted borrow, not yet returned)
                string memberIdStr = Session["MemberID"]?.ToString() ?? "";
                if (string.IsNullOrEmpty(memberIdStr))
                {
                    ShowError("Session expired. Please log in again.");
                ...
                // Only allow return request if book is actively borrowed
                ...
                // Submit return request — insert a NEW transaction row
                // The original borrow row stays as-is until admin accepts the return
                string isbn = dt.Rows[0]["ISBN"].ToString();

                // Check there isn't already a pending return for this borrow
                ...
                // Update the existing borrow row to flag it as a pending return
                // (simpler than a new row — admin sees RequestType=Return, RequestStatus=Pending)
        ...
        // ── Badge helpers (called from ASPX) ─────────────────────────────────
        ...
        protected void btnChangePassword_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                bool success = DatabaseHelper.ChangePassword(userId, current, newPass);
