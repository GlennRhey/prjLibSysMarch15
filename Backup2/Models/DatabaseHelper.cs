    ...
    public class DatabaseHelper
    {
            ...
            string subject, string message)
        {
            string query = @"
                ...
                (NotificationType, Recipient, Subject, Message, CreatedAt)
                VALUES (@Type, @Recipient, @Subject, @Message, @CreatedAt)";
        ...
        public static void SendDueDateReminders()
        {
            ...
            string query = @"
                SELECT t.BorrowID, m.Email, b.Title, t.DueDate, m.FullName
                ...
                AND (t.DueDateReminderSent IS NULL OR t.DueDateReminderSent = 0)";
            ...
            foreach (DataRow row in dueBooks.Rows)
            {
                ...
                // Mark reminder as sent (if the column exists)
                try
                {
                    ExecuteNonQuery("UPDATE tblTransactions SET DueDateReminderSent = 1 WHERE BorrowID = @BorrowID",
